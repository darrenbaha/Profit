...
public class Workouts extends Activity implements AdapterView.OnItemClickListener {
    ...
    public boolean onContextItemSelected(final MenuItem item) {
        System.out.println(workoutIDs);
        final ArrayAdapter<String> adapter = new ArrayAdapter<String>(this,android.R.layout.simple_list_item_1,workouts);
        work.setAdapter(adapter);
        ...
        //}
            ...
            System.out.println("Rename is running");


            LayoutInflater inflater = Workouts.this.getLayoutInflater();
            final AlertDialog alertDialog = new AlertDialog.Builder(this).create();
            alertDialog.setView(inflater.inflate(R.layout.rename_workout, null));
            alertDialog.setButton(DialogInterface.BUTTON_NEUTRAL, "Save", new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
                    final String renameWorkout = ((EditText) alertDialog.findViewById(R.id.rename_field)).getText().toString();
                    if (!renameWorkout.trim().isEmpty()) {

                        ParseQuery query = ParseQuery.getQuery("Workouts");
                        AdapterView.AdapterContextMenuInfo info = (AdapterView.AdapterContextMenuInfo) item.getMenuInfo();
                        final int position = info.position;
                        query.whereEqualTo("objectId", workoutIDs.get(position));
                        ...
                        query.getInBackground(workoutIDs.get(position), new GetCallback<ParseObject>() {
                            ...
                            public void done(ParseObject parseObject, ParseException e) {
                                if (e == null) {
                                    ...
                                    System.out.println("id is: " + workoutIDs.get(position));
                                    System.out.println("Text input is: " + renameWorkout);
                                    parseObject.put("WorkoutName", renameWorkout);
                                    parseObject.saveInBackground();
                                    getWorkouts();
                                    adapter.notifyDataSetChanged();
                                } else {
                                    Toast.makeText(Workouts.this, "Could not rename workout", Toast.LENGTH_SHORT).show();
                                    System.out.println("Exception renaming workout: ");
                                    e.printStackTrace();
                                }
                            }
                        });
            ...
            alertDialog.show();
         /*   alertDialog.findViewById(R.id.rename_field).requestFocus();
            InputMethodManager imm = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
            imm.showSoftInput(alertDialog.findViewById(R.id.rename_field), InputMethodManager.SHOW_IMPLICIT);
            */
            Button b = alertDialog.getButton(DialogInterface.BUTTON_NEUTRAL);
            b.setBackgroundDrawable(this.getResources().getDrawable(R.drawable.buttongeneric));
